"""Executable all-layers OVERSEE scenario runner.

Purpose:
    Execute one scenario through the complete OVERSEE Layer 1 to Layer 5 path.

Architectural role:
    This script is the main technical-review entry point for the executable
    demo. It orchestrates scenario loading, predictive-alert preparation, the
    scenario-backed enterprise API boundary, the five OVERSEE layers and the
    persistence of reviewable artifacts.

Inputs:
    A scenario identifier that resolves to one executable scenario JSON file.

Outputs:
    A timestamped output directory containing layer-by-layer JSON/Markdown
    artifacts. Only selected artifacts are named output_layer_X because they
    represent the main output of one layer and the input to the next layer.

Important note:
    The generated JSON and Markdown files are not the implementation logic.
    They are the persisted audit trail produced by the Python layer execution.
"""
...
def all_layers_cli_main() -> None:
    ...
    This is the CLI entry point used by reviewers or developers who want to run
    one scenario from the terminal. It parses the scenario identifier, lists
    available scenarios when requested, executes the complete all-layers path and
    prints the compact execution summary returned by the runner.
    
    This function belongs to the runner/orchestration boundary, not to a single
    OVERSEE layer.
    """
...
def run_all_layers_scenario_execution(scenario_id: str) -> dict[str, Any]:
    ...
    Execution sequence:
        1. Load the selected executable scenario.
        2. Validate its executable input structure.
        3. Build the predictive alert request.
        4. Create the scenario-backed enterprise API client.
        5. Execute Layer 1 evidence intake and validation.
        6. Build the canonical case context and execute Layer 2 contextualization.
        7. Execute Layer 3 case lifecycle management.
        8. Execute Layer 4 DMN-like decision and recommendation logic.
        9. Execute Layer 5 governed packaging and traceability.
        10. Persist reviewable artifacts to the output directory.
    
    The function deliberately separates Python execution from artifact persistence:
    ...
    write_layer_artifact_json(paths["scenario"], scenario.raw)
    write_layer_artifact_json(paths["predictive_alert_request"], alert_request)
    write_layer_artifact_json(paths["received_predictive_alert"], layer1_result.received_alert.to_dict())
    write_layer_artifact_json(paths["enterprise_api_calls"], layer1_result.enterprise_api_calls)
    write_layer_artifact_json(paths["aggregated_evidence_package"], layer1_result.evidence_package.to_dict())
    write_layer_artifact_json(paths["validation_report"], layer1_result.validation_report)
    write_layer_artifact_json(paths["canonical_case_context"], canonical_context.to_dict())
    write_layer_artifact_json(
        paths["contextualization_rule_trace"],
        [rule.to_dict() for rule in layer2_result.rule_trace],
    )
    write_layer_artifact_json(paths["layer2_contextualization_result"], layer2_result.to_dict())
    write_layer_artifact_json(paths["case_lifecycle_trace"], case_state.lifecycle_trace())
    write_layer_artifact_json(paths["case_management_state"], case_state.to_dict())
    write_layer_artifact_json(paths["dmn_decision_evaluation"], rule_evaluation.to_dict())
    write_layer_artifact_json(paths["recommendation_path_outputs"], recommendation_bundle.to_dict())
    write_layer_artifact_json(paths["governed_recommendation_package"], governed_package.to_dict())
    write_layer_artifact_json(paths["traceability_index"], governed_package.traceability_index)
    ...
    write_layer_artifact_json(paths["execution_manifest"], manifest)
...
def run_scenario_all_layers(scenario_id: str) -> dict[str, Any]:
    """Backward-compatible all-layers entry point for tests and integrations.
    
    The preferred implementation name is run_all_layers_scenario_execution().
    This wrapper remains available because existing tests, adapters and reports
    import run_scenario_all_layers(). It delegates directly to the preferred
    implementation without changing behavior.
    """

    return run_all_layers_scenario_execution(scenario_id)

def print_all_layers_available_scenarios() -> None:
    """Print all executable scenarios available to the all-layers runner.
    
    This helper is used by the CLI to show the scenario library. It does not
    execute any OVERSEE layer and does not generate artifacts.
    """
...
def write_layer_artifact_json(path: Path, payload: Any) -> None:
    """Write one JSON layer artifact to disk.
    
    This helper persists already-computed Python objects as JSON files for review,
    traceability and auditability. It does not contain OVERSEE decision logic; it
    only serializes the artifacts produced by the runner.
    """
...
def serialize_layer_artifact_default(value: Any) -> Any:
    """Convert common OVERSEE layer objects into JSON-serializable values.
    
    The all-layers runner receives dataclasses, objects with to_dict() methods and
    standard Python values. This serializer keeps artifact writing centralized and
...
def build_layer5_scenario_execution_summary(
    *,
    ...
    """Build the Layer 5 human-readable scenario execution summary.
    
    The summary is a reviewer-facing Markdown artifact that points to the most
    important generated files and records the final governed recommendation fields.
    It complements the JSON artifacts but does not replace them.
    """
