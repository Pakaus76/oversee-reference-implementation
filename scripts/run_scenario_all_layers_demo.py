...
Architectural role:
    This script is the main code-review entry point for the executable demo.
    It orchestrates scenario loading, predictive-alert preparation, the
    scenario-backed enterprise API boundary, all five OVERSEE layers, and
    artifact persistence.

Important note:
    The JSON and Markdown files generated by this runner are not the
    implementation logic. They are the persisted audit trail produced by the
...
def run_scenario_all_layers(scenario_id: str) -> dict[str, Any]:
    ...
    The preferred implementation name is
    run_all_layers_scenario_execution(). This wrapper is intentionally kept
    because existing tests, adapters and reports import run_scenario_all_layers().
    '''
