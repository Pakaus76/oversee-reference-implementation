"""Run Fernando all-layers OVERSEE end-to-end demo.

This script closes the Fernando-aligned five-layer flow:
...
def main() -> None:
    """Run the Fernando all-layers end-to-end demo and persist inspectable outputs."""

    load_powershell_env_file(PROJECT_ROOT / ".env")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = PROJECT_ROOT / "outputs" / f"fernando_all_layers_demo_{timestamp}"
    ...
    print("Fernando all-layers OVERSEE demo completed.")
    print()
    print(
        json.dumps(
            {
                "fernando_all_layers_demo_completed": True,
                "compressor_case_id": canonical_context.case_id,
...
def _build_layer3_summary(
    ...
    return (
        "# Fernando Layer 3 case lifecycle summary\n\n"
        "## Purpose\n\n"
...
def _build_layer4_summary(
    *,
    ...
    return (
        "# Fernando Layer 4 decision and recommendation summary\n\n"
        "## Purpose\n\n"
