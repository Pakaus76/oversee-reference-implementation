...
The adapter calls the existing paper-aligned Layer 2 script as a subprocess.
...
def run_real_layer2(state: DemoRunState) -> dict[str, Any]:
    ...
    repo_root = Path.cwd()
    script_path = repo_root / "scripts" / "run_layer2_paper_aligned_demo.py"
...
def _find_layer2_output_dirs(outputs_dir: Path) -> set[Path]:
    """Return known Layer 2 paper-aligned output directories."""
    if not outputs_dir.exists():
        return set()

    return {
        path.resolve()
        for path in outputs_dir.glob("paper_aligned_layer2_demo_*")
        if path.is_dir()
    }
