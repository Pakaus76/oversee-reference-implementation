...
This is the main demo case and the one to use with Fernando first.
...
For this scenario, the walkthrough executes the real Fernando-aligned layer scripts:

```text
scripts/run_layer1_fernando_demo.py
scripts/run_layer2_fernando_demo.py
scripts/run_layer3_fernando_demo.py
scripts/run_layer4_fernando_demo.py
scripts/run_layer5_fernando_demo.py
```
...
Use it at the beginning if Fernando asks what cases are available.
...
Use this in the meeting with Fernando.
...
It is also useful if Fernando wants to see the complete flow once without interruption.
...
Use this only if Fernando asks:
...
Use it after the main COMP-001 demo if Fernando asks whether the architecture can represent a less urgent case.
...
Use it if Fernando asks whether the architecture can represent constraints, not just simple positive decisions.
...
Get-ChildItem outputs -Directory |
    Where-Object {
        ...
        $_.Name -like "fernando_layer1_demo_*" -or
        $_.Name -like "fernando_layer2_demo_*" -or
        $_.Name -like "fernando_layer3_demo_*" -or
        $_.Name -like "fernando_layer4_demo_*" -or
        $_.Name -like "fernando_layer5_demo_*"
    } |
...
# Recommended live sequence with Fernando
...
## Step 4 — If Fernando asks for evidence
...
## Step 5 — If Fernando asks for other cases
