"""
Module: grounded_model_payload.py

Purpose:
    Define the structured payload contract expected from the future real
    grounded implementation of grounded model path.

Architectural role:
    This module isolates the parsing and validation discipline of the model
    response used by grounded model path. Its role is intentionally narrow:
    - define the typed payload expected from the model
    - parse one JSON-like text response
    - validate the minimum structural integrity of that response before
      grounded model path builds the final repository Recommendation

Thesis traceability:
    - Chapter 6: Controlled progression from model-backed recommendation toward
      grounded recommendation through explicit structured validation
    - Chapter 7: First typed payload contract for the real grounded E condition
    - Chapter 8: Better comparability and reproducibility of D-versus-E
      evaluation through disciplined output validation

Inputs:
    - Raw text returned by the OpenAI helper for grounded model path

Outputs:
    - GroundedModelPayload instances containing validated recommendation fields

Key assumptions:
    - The first grounded model path condition should require a narrow structured response.
    - The model is expected to return only one compact JSON object.
    - Structural validation must happen before the repository builds the final
      Recommendation output.
    - This module validates structure and bounded field integrity, not broader
      grounding plausibility. Grounded-use plausibility should be checked in the
      E condition where the evidence bundle is available.

Dependencies:
    - dataclasses
    - json
    - typing
    - internal domain priority vocabulary

Notes:
    - This module intentionally accepts a narrow JSON payload only.
    - This module does not call the model by itself.
    - This module does not implement retrieval, fallback logic, or second-pass
      deliberation.
"""

from __future__ import annotations

from dataclasses import dataclass
import json
from typing import Any, Mapping

from oversee.domain import PriorityLevel


class GroundedModelPayloadError(ValueError):
    """
    Represent a validation error in the structured payload returned for E.

    Why this exists:
        Real grounded model path introduces grounded model-generated content. The
        repository therefore needs a specific and readable validation error when
        the model output is missing, malformed, or outside the accepted bounded
        contract.
    """


@dataclass(frozen=True, slots=True)
class GroundedModelPayload:
    """
    Store the validated structured response expected from the grounded model path model call.

    Args:
        priority: Repository-compatible priority label.
        action: Non-empty recommended action text.
        rationale: Non-empty justification text.

    Returns:
        GroundedModelPayload: Typed validated payload ready for grounded model path.

    Raises:
        GroundedModelPayloadError: If any field is empty or invalid.
    """

    priority: str
    action: str
    rationale: str

    def __post_init__(self) -> None:
        """
        Validate the minimum integrity of the typed payload.
        """
        allowed_priorities = {level.value for level in PriorityLevel}

        normalized_priority = self.priority.strip()
        normalized_action = self.action.strip()
        normalized_rationale = self.rationale.strip()

        if not normalized_priority:
            raise GroundedModelPayloadError("priority must not be empty.")

        if normalized_priority not in allowed_priorities:
            raise GroundedModelPayloadError(
                "priority must belong to the repository priority vocabulary."
            )

        if not normalized_action:
            raise GroundedModelPayloadError("action must not be empty.")

        if not normalized_rationale:
            raise GroundedModelPayloadError("rationale must not be empty.")


def _strip_optional_code_fences(raw_text: str) -> str:
    """
    Remove one optional surrounding Markdown code fence from model output.

    Args:
        raw_text: Raw text returned by the model helper.

    Returns:
        str: Text without one outer fenced block when present.
    """
    normalized_text = raw_text.strip()

    if not normalized_text.startswith("```"):
        return normalized_text

    lines = normalized_text.splitlines()

    if len(lines) < 3:
        return normalized_text

    if not lines[-1].strip().startswith("```"):
        return normalized_text

    inner_lines = lines[1:-1]
    return "\n".join(inner_lines).strip()


def _parse_json_object(raw_text: str) -> Mapping[str, Any]:
    """
    Parse the raw model text into one JSON object.

    Args:
        raw_text: Raw text returned by the model helper.

    Returns:
        Mapping[str, Any]: Parsed top-level JSON object.

    Raises:
        GroundedModelPayloadError: If the text is empty, invalid JSON, or not an
            object.
    """
    if not raw_text.strip():
        raise GroundedModelPayloadError("Model output text must not be empty.")

    normalized_text = _strip_optional_code_fences(raw_text)

    try:
        parsed_payload = json.loads(normalized_text)
    except json.JSONDecodeError as exc:
        raise GroundedModelPayloadError(
            "Model output is not valid JSON for grounded model path."
        ) from exc

    if not isinstance(parsed_payload, dict):
        raise GroundedModelPayloadError(
            "Model output for grounded model path must be a JSON object."
        )

    return parsed_payload


def _require_string_field(
    payload: Mapping[str, Any],
    field_name: str,
) -> str:
    """
    Read one required string field from the parsed payload.

    Args:
        payload: Parsed JSON object.
        field_name: Name of the required field.

    Returns:
        str: Normalized non-empty string value.

    Raises:
        GroundedModelPayloadError: If the field is missing, not a string, or empty.
    """
    if field_name not in payload:
        raise GroundedModelPayloadError(
            f"Missing required field '{field_name}' in grounded model path payload."
        )

    raw_value = payload[field_name]

    if not isinstance(raw_value, str):
        raise GroundedModelPayloadError(
            f"Field '{field_name}' in grounded model path payload must be a string."
        )

    normalized_value = raw_value.strip()

    if not normalized_value:
        raise GroundedModelPayloadError(
            f"Field '{field_name}' in grounded model path payload must not be empty."
        )

    return normalized_value


def parse_grounded_model_payload(raw_text: str) -> GroundedModelPayload:
    """
    Parse and validate the structured payload returned for grounded model path.

    Args:
        raw_text: Raw text returned by the OpenAI helper.

    Returns:
        GroundedModelPayload: Typed validated payload ready for grounded model path.

    Raises:
        GroundedModelPayloadError: If the raw text cannot be parsed into a valid
            grounded model payload.

    Validation scope:
        - top-level JSON object required
        - required fields: priority, action, rationale
        - non-empty string fields
        - repository-compatible priority vocabulary
    """
    payload = _parse_json_object(raw_text)

    priority = _require_string_field(payload, "priority")
    action = _require_string_field(payload, "action")
    rationale = _require_string_field(payload, "rationale")

    return GroundedModelPayload(
        priority=priority,
        action=action,
        rationale=rationale,
    )
